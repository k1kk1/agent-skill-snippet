...
既定では **Dock に出る通常のアプリ**として動き、起動すると Manage Recipes が開く。
メニューバーのアイコンも常駐するので、どちらからでも Recipe を実行できる。

Dock に出したくない場合は Settings → General の「**Dock に表示する**」を OFF にする
（メニューバー常駐だけになる。再起動は不要）。ウィンドウを全部閉じても常駐は続く。
...
- 引数: Recipe に定義した `{{url}}` など（型は **String / Multiline / URL / 選択** の 4 つ）
  ...
  - **複数選択**にすると、選んだ値を `全体像, API` のように連結して渡す（順番は候補の並び順に揃う）
